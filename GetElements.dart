...
main() async {
  ...
  try {
    ...
    gridList.forEach((element) {
      print('Element ${element.title} ${element.x} ${element.y}');
      if (element.title.isNotEmpty) {
        elements.add(element.toJson(extract: extractMap[element.title]!));
      }else{
        elements.add(null);
      }
    });
    ...
    print('Failed to write to output file ${outputFile.path}');
    print(e);
  }
}
...
Future<Map<String, String>> getExtractMap() async {
  ...
  print("Len Uchiha: ${kElementsList.length}");
  do {
    var extractElements = remainingElements.take(count);
    remainingElements = remainingElements.skip(count);

    print('extractElements ${remainingElements.length} : ${getUrl(extractElements)}');

    var response = await http.get(getUrl(extractElements));

    if(response.statusCode != 200) {
      print('Failed to fetch extracts: ${response.statusCode}');
      return extractsMap;
    }
...
class Element {
  ...
  Category get category => Category.values.firstWhere(
          (category) => kCategoryMap[category]?.contains(symbol) ?? false);
